...
function generateProject (projectDir, name, namespace) {

    const oldCwd = process.cwd();

    const env = yeoman.createEnv();
    const generatorPath = path.join(__dirname, 'generator-react-native-tizen');
    env.register(generatorPath, 'react:tizen');
    env.run('react:tizen', {'skip-install':true, 'force':true},
        err => {
            if(err) {throw err;}
    });
}
