...
namespace ReactNative.Bridge
{
    ...
    public sealed class NativeModuleRegistry
    {
        ...
        {
            using (RNTracer.Trace(RNTracer.TRACE_TAG_REACT_BRIDGE, "CreateJSON").Start())
            {
        ...
        internal void NotifyReactInstanceInitialize()
        {
            ...
            using (RNTracer.Trace(RNTracer.TRACE_TAG_REACT_BRIDGE, "NativeModuleRegistry_NotifyReactInstanceInitialize").Start())
            {
        ...
        {
            DispatcherHelpers.AssertOnDispatcher();
            using (RNTracer.Trace(RNTracer.TRACE_TAG_REACT_BRIDGE, "NativeModuleRegistry_NotifyReactInstanceDestroy").Start())
            {
                foreach (var module in _moduleInstances.Values)
                {
        ...
        class ModuleDefinition
        {
            ...
            public void WriteModuleDescription(JsonWriter writer)
            {
                ...
                //TODO: When first time use this function, used more than 1 second, need further check
