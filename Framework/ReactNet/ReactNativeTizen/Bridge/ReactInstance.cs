...
namespace ReactNative.Bridge
{
    ...
    class ReactInstance : IReactInstance, IAsyncDisposable
    {
        ...
        public void InvokeCallback(int callbackId, JArray arguments)
        {
            if (IsDisposed)
            {
                RNTracer.Write(ReactConstants.Tag, "Invoking JS callback after bridge has been destroyed.");
                return;
            }
            ...
            QueueConfiguration.JavaScriptQueueThread.RunOnQueue(() =>
            {
                ...
                using (RNTracer.Trace(RNTracer.TRACE_TAG_REACT_BRIDGE, "<callback>").Start())
                {
        ...
        public /* TODO: internal? */ void InvokeFunction(string module, string method, JArray arguments, string tracingName)
        {
            QueueConfiguration.JavaScriptQueueThread.RunOnQueue(() =>
            {
                ...
                using (RNTracer.Trace(RNTracer.TRACE_TAG_REACT_BRIDGE, tracingName).Start())
                {
                    if (_bridge == null)
                    {
        ...
        private string BuildModulesConfig()
        {
            ...
            try
            {
                ...
                //Log.Info(ReactConstants.Tag, ">>>>>>>> BuildModulesConfig=" + stringWriter.ToString());
        ...
        private void CollectGC(string strInput)
        {
            if (strInput == "didCompleteNetworkResponse")
            {
        ...
        class NativeModulesReactCallback : IReactCallback
        {
            ...
            public void OnBatchComplete()
            {
                ...
                if (!_parent.IsDisposed)
                {
                    using (RNTracer.Trace(RNTracer.TRACE_TAG_REACT_BRIDGE, "OnBatchComplete").Start())
                    {
