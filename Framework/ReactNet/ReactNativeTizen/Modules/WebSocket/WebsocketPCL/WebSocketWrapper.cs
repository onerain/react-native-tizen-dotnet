...
namespace Websockets.Net
{
    ...
    internal class WebSocketWrapper : IDisposable
    {
        ...
        public event Action<MessageEventArgs, WebSocketWrapper> MessageReceived;
        ...
        public async Task SendMessage(byte[] message)
        {
            ...
            try
            {
                ...
                using (await _asyncLock.LockAsync())
                {
                    await _ws.SendAsync(new ArraySegment<byte>(messageBuffer, 0, messageBuffer.Length), WebSocketMessageType.Binary, true, _cancellationToken);
                }
            }
            catch (Exception ex)
            {
        ...
        private async void StartListen()
        {
            ...
            try
            {
                while (_ws.State == WebSocketState.Open)
                {
                    ...
                    {
                        //byte[] byteArray = Encoding.ASCII.GetBytes(stringResult.ToString());
                        messageargs.IsText = false;
                    }
                    else
                    {
                        messageargs.IsText = true;
                    }
                    messageargs.Data = stringResult.ToString();
                    CallOnMessage(messageargs);
                }
            }
            catch
            {
        ...
        private void CallOnMessage(MessageEventArgs Result)
        {
            var ev = MessageReceived;
            if (ev != null)
                RunInTask(() => ev(Result, this));
        }
