...
namespace ReactNative.UIManager.Events
{
    ...
    /// So dispatch events directly.
    /// 
    /// Plan TODO:
    /// For forbidden too much event sent to JavaScript,
    /// If current_time - last_dispach_time less than 20ms, Just save event in eventStaging
    /// When FrameTimer event come(each 20ms a event), process event in eventStaging.
    /// 
    /// </summary>
    /// <remarks>
    ...
    public class EventDispatcher : ILifecycleEventListener
    {
        ...
        public void OnResume()
        {
            ...
            //[TODO] CompositionTarget.Rendering += ScheduleDispatcherSafe;
        }
        ...
        private void ScheduleDispatch(object sender, object e)
        {
            ...
            var activity = RNTracer.Trace(RNTracer.TRACE_TAG_REACT_BRIDGE, "ScheduleDispatch").Start();
        ...
        private void DispatchEvents(IDisposable activity)
        {
            ...
            using (RNTracer.Trace(RNTracer.TRACE_TAG_REACT_BRIDGE, "DispatchEvents").Start())
            {
